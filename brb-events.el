  ...
  (let* ((actions (use-brb-events-actions))
    ...
    (vui-vstack
     ...
     (vui-table
      ...
      :rows (->> participants
                 (--map
                  (let ((past-count (-count (lambda (other)
                    ...
                    (list (vui-button (vulpea-note-title it)
                          ...
                          (cond
                           ...
                           (t (vui-text "new" :face 'shadow)))
                          (or (vulpea-note-meta-get it "tasting level") "")
                          (or (vulpea-note-meta-get it "wix") "")
                          (number-to-string past-count)
                          (number-to-string
                           (-count (lambda (other)
                                     (string-equal (vulpea-note-id it)
                                                   (vulpea-note-id other)))
                                   participants-all)))))
                 (--sort (> (string-to-number (nth 4 it))
                            (string-to-number (nth 4 other)))))
      :border :ascii)
     (vui-newline))))
...
(vui-defcomponent brb-events-stats-wines (events summary-tbl)
  ...
  (let* ((today (format-time-string "%F" (current-time)))
         ...
         (wine-rows
          (--map (let ((wavg-str (nth 5 it)))
                   (if (string= wavg-str "-")
                       it
                     (let* ((val (string-to-number wavg-str))
                            (face (cond ((>= val 4.0) 'success)
                                        ((< val 3.7) 'error)
                                        (t nil))))
                       (if face
                           (-replace-at 5 (vui-text wavg-str :face face) it)
                         it))))
                 wine-rows-sorted)))
    (vui-vstack
